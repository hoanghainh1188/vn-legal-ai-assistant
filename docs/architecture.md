...
| Vector DB | Postgres + pgvector (kho quan hệ thống nhất) | 5432 |
...
| Ingest | `scripts/init_db.py` | Áp `app/db/schema.sql` vào Postgres (idempotent) |
| Ingest | `scripts/ingest.py` | **1 lệnh**: schema → fetch → text → chunk → embed → upsert |
